...
<Agent -> Environment>
online update / episodic update 가 가능하기 때문에 keras-rl 코드의 전체 구조 유지
single-agent <-> multi-agent 호환 가능하도록 해야 함

1. discrete action: one-hot vector로 전달 
2. continuous action: numeric vector로 전달
3. agent.forward
    3.1 input: state
    3.2 output: Q values / probablity / numeric vector
    3.3 policy를 이용하여 one-hot vector로 변환
4. agent.backward
    4.1 online update / episodic update를 가능하도록 구성 
    4.2 on-policy / off-policy 호환 가능하도록
    4.3 target 계산하는 부분 모듈화 (n-step target 계산 가능하도록)
    4.4 update는 가급적 train_on_batch를 이용
        (sample-wise loss 함수를 정의만 해주면 가능)
    
5. 개발해햐할 agent
    (discrete action)
    5.1 DQN agent: DQN, Double DQN, dueling DQN
    5.2 Policy gradient agent: A2C    

    (continuous action)
    5.3 DDPG

6. Asynchronous method agent
    6.1 위의 내용을 다 커버 할 수 있는 general framework 형식으로 구성해야 함
    6.2 Environment 구성의 이슈?
...
class Agent(object):
    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
        ...
        try:
            while self.step < nb_steps:
                ...
                if observation is None:  # start of a new episode
                    ...
                    while step_rnd <= nb_random_start_steps:
                        ...
                        if start_step_policy is None:
                            ...
                            # TODO env.action_space.sample 이용할 수 있도록 수정
                            actions = []
                ...
                # 매 스텝마다 실행되는 backward
                # s, a, r, s' 를 memory에 저장하는 과정이 포함되어 있음
    ...
    def test(self, env, nb_episodes=1, action_repetition=1, callbacks=None, visualize=True,
        ...
        while episode < nb_episodes:
            ...
            if gvar.service_flag == 0:
                time.sleep(0.)
                # logger.info("The thread for RL is sleeping...[case#1]")
                continue
