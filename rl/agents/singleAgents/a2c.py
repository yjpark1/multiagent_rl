...
class DiscreteA2CAgent(Agent):
    ...
    def critic_optimizer(self):
        ...
        update = self.critic.optimizer.get_updates(loss=loss, params=self.critic.trainable_weights)
    ...
    def actor_optimizer(self):
        ...
        update = self.actor.optimizer.get_updates(loss=loss, params=self.actor.trainable_weights)
    ...
    def backward(self, reward, terminal=False):
        ...
        metrics_critic = self.critic_train_on_batch([observations, Rs])[0]
        advantages = Rs - Vs
        metrics_actor = self.actor_train_on_batch([observations, actions, advantages])[0]
        metrics = [metrics_critic, np.mean(metrics_actor)]
