...
class MA_DDPGAgent(Agent):
    """Write me
    """
    def __init__(self, nb_agents, nb_actions, actor, critic, critic_action_input, memory,
    ...
    def select_action(self, state):
        ...
        # process categorical action and continuous action
        onehot = np.zeros((self.nb_agents, 3))  # 3 categorical actions
        onehot[np.arange(self.nb_agents), np.argmax(action[:, :, 0:3], axis=-1)] = 1.
        action[:, :, 0:3] = onehot
        return action
    ...
    def forward(self, observation):
        ...
        # print('action: ', action)
        return action # np.argmax(action, axis=-1)
    ...
    def backward(self, reward, terminal=False):
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.nb_steps_warmup_critic:
                ...
                assert target_q_values.shape == (self.batch_size, self.nb_agents, 1)
                ...
                # print("terminal1_batch shape1: ", terminal1_batch.shape)
                terminal1_batch = np.repeat(terminal1_batch, repeats=self.nb_agents, axis=-1)  #
                terminal1_batch = np.expand_dims(terminal1_batch, axis=-1)  #
                # print("terminal1_batch shape2: ", terminal1_batch.shape)
                ...
                targets = (reward_batch + discounted_reward_batch).reshape(self.batch_size, self.nb_agents, 1)
                ...
                metrics = self.critic.train_on_batch(x=state0_batch_with_action, y=targets)
                if self.processor is not None:
