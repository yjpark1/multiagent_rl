...
class MA_DDPGAgent(Agent):
    ...
    def forward(self, observation):
        ...
        # prob_noise = np.random.multinomial(10, pvals=[1 / self.nb_actions] * self.nb_actions, size=(self.nb_agents,))
        # prob_noise = prob_noise.astype('float32')
        # prob_noise *= 0.1
        #
        # action_probs += prob_noise * 0.1
        # action_probs = action_probs / np.sum(action_probs, axis=-1).reshape(6, 1)
        #
        # action_probs = np.expand_dims(action_probs, axis=0)

        # Book-keeping.
        self.recent_observation = observation
        self.recent_action = action_probs
